...
const ExamscheduleDashboard = () => {
  ...
  const handleExportCSV = async (e) => {
    ...
    const resultExport = await dispatch(
      getAllExamschedules({ page: 1, pageSize: 9999999 })
    )
    const allExamSchedule = resultExport?.payload.data.data
    await setExportData(allExamSchedule)
    ...
    await dispatch(getAllExamschedules(param))
  }
  ...
  useEffect(() => {
    if (
      dataexs?.loadings[examscheduleTypes.GET_EXAMSCHEDULES] ||
      dataexs?.loadings[examscheduleTypes.CREATE_EXAMSCHEDULE] ||
      dataexs?.loadings[examscheduleTypes.UPDATE_EXAMSCHEDULE] ||
      dataexs?.loadings[examscheduleTypes.DELETE_EXAMSCHEDULE]
    )
      setLoading(true)
    else setLoading(false)
  }, [dataexs, param])
