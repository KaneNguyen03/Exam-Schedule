...
const examslotSlice = createSlice({
  ...
  extraReducers: {
    ...
    [updateExamslot.fulfilled]: (state, payload) => {
      ...
      ].data.data.findIndex((c) => c.examSlotId === payload.meta.arg.examSlotId)
    ...
    [createExamslot.fulfilled]: (state, payload) => {
      ...
      const temp = payload.meta.arg
      state.contents[examslotTypes.GET_EXAMSLOTS].data.data.push(temp)
